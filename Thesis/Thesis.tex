...
{\LARGE{\bf Abstract}}\\
\vspace{9mm}
%L'Approximate Bayesian Approach ha molti punti a sfavore tra cui il fatto che � computazionalmente expensive.
...
%L'Approximate Bayesian Approach Computation � computational expensive e indica la discussione su stackexchange.
%+punti del Professore.
%l'importante � che abbia evidenziato bene quali sono le parti di notivit� che ha fatto lei, fra cui: 1) l'algoritmo misto 2) l'applicazione delle wavelet per l'analisi 3) l'uso della rete neurale per la predizione 4) l'impostazione della CME per il nfkb.
...
As mentioned in section \ref{theonestepdiscussion} when discussing about one-step processes, from a mathematical point of view, constitutive expression can be seen as a particular case of a one-step process since such molecular reactions are random Poissonian birth-death processes. This is represented in Fig.\ref{theonestepRNA} that reminds to Fig.\ref{theonestep} of the previous chapter.
...
This is not the case of proteins distribution. The number of proteins produced yields a \emph{scaling law} since they are dependent on both mRNA birth and death as well as translation rates \cite{Article}. Therefore proteins have non-Poisson distributions albeit their scaling is also roughly Poissonian.
...
Fig. \ref{firstmodelka1ki0.5timeplot} and Fig. \ref{firstmodelka0.1ki1timeplot} show the number of RNA and molecules produced, using the SSA simulation, by considering respectively two types of regulation: one when the gene spents more time to be in active state than inative state whereas the other is the opposite situation.
...
The other two cases that we are going to simulate are the autorepressor case and the toggle-switch. We can anticipate how the pattern of molecules produced related to the case of the gene that spents more time to be inactive (see Fig. \ref{firstmodelka0.1ki1timeplot}) is similar to the autorepressor case that we are going to explore.
\newpage

\section{The autorepressor simulation}

Fig.\ref{autorepressortimeplot} shows the typical pattern of the number of transcript and translated molecules produced in a genetic autorepressor circuit.
...
\section{The Toggle-Switch simulation}
...
The shape of such autocorrelation functions in Fig.\ref{ToggleSwitchACFnlags10000dt100nsimulations64} would resemble to the first model case, however in the that case, the error bars where indeed much smaller indicating a smaller variation into the autocorrelation trends for each random seed and the median values lay exactly on the horizontal line corresponding to the zero value y-axis.
...
\chapter{Artificial Neural Networks (ANN) as data-driven modeling}\label{ArtificialNeuralNetwork}
...
In this thesis we use Artificial Neural Networks (ANN) as a method of data-driven modeling class. We use as input data the autocorrelation function whose shape depends on the system parameters and the output data are, indeed, the constant rates of reactions. Hence, it is a matter of building a \emph{multi-output regression model} using a supervised deep neural network. Then this can be exploited for the purpose of state estimation.
...
Then the \emph{output} layer has a number of nodes equal to the output variables that the neural network has to predict. In our case, since we aim to obtain the model paramaters starting from the autocorrelation function, if the parameters are 4 (for instance the constants rate of the numeber of RNAs molecules increase and degradation and the same analogously for proteins in the simple first model analysed in chapter \ref{models}), the the output layer will have 4 nodes.
...
\begin{itemize}
    \item one is the \emph{activation function}. As the name suggests, it ``activates'' the
    output that the weighted sum of the inputs is transformed so that the output is limited into a determined range of values. In this case we use the \emph{ReLU} function for each hidden layer and output layer. The rectified linear activation function (or ReLU) for short is a piecewise linear function that will output the input directly if it is positive, otherwise, it will output zero (see Fig.\ref{ReLU}). 

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{ReLUactivationfunction.png}
    \caption{Sketch of ReLU activation function. Image from \cite{ANNBrainTumorsNMR}}
    \label{ReLU}
    \end{figure}

    Indeed we are going to predict values that are posive numbers (i.e. the model parameters) hence this is suitable for our context.
    
    \item One other ingredient is \emph{the optimization algorithm}. The learning strategy of our neural network is based on backpropagation algorithm that updates all the weights (i.e. connections between neurons) in order to minimize \emph{an error function} which characterizes the comparison between the network output and the ground truth in supervised models. The variation in time of the weights is given by: 
    \begin{equation}
     \dot{w_{i}}=\frac{dE}{dw}
    \end{equation}
    
    where E is the error function that is also refered to as energy function in analogy to a physical system in which forces act to minimize the energy. There are different optimizers to make the network evolve, i.e.
    algorithms used to change the attributes of a neural network such as weights and learning rate in order to reach the optimal solution and minimize the loss. In this study we use the \emph{Adam} as optimization algorithm that deals with the challenge of choosing the proper learning rate (in a classic gradient descent method, if it is too small leads to painfully slow convergence, if too large can hinder convergence) and of avoiding getting trapped in the suboptimal local minima. Its strategy is to accelarate the learning rate but it dampens oscillations towards local minimum. It combines the heuristics of two methods called Momentum and RMSProp.
    
    \item As regards the error function instead, since we are building a regressor model, we use the \emph{Mean Absolute Error}. The mean absolute error of a model with respect to a test set is the mean of the absolute values of the individual prediction errors on over all instances in the test set. Each prediction error is the difference between the true value and the predicted value for the instance \cite{MAE}.
    \begin{equation}
     mae = \frac{\sum_{n=1}^{n} abs(y_{i}-\lambda(x_{i})}{n}
    \end{equation}
    
    where $y_{i}$ is the true target value for test instance $x_{i}$, $\lambda(x_{i})$ is the predicted target value for test instance $x_{i}$, and \emph{n} is the number of test instances.
...
\begin{itemize}
 \item the \emph{batch size}:  computing the gradient on the whole dataset is computationally very complex so nowadays the choice is to devide the dataset into batches, that is \emph{the number of samples to work through before updating the internal model paramaters} (i.e. weights). If we set a batch size equal to 128, the model weigths will be updated after each batch of 128 samples.
 \item \emph{number of epochs}: it defines the number of times that the learning algorithm will work through the entire training dataset. One epoch means that each sample in the training dataset has had an opportunity to update the internal model parameters.
 \item \emph{dropout}: since there can be problems of overfitting so that the network learns the input data without being able to generalize or also the problem to get stuck in local minima of the energy function that implies a not optimal learning of the network, there are some regularization procedures to overcome these problems, one of which is perturbing the architecture of the network (dropout). This method consists in removing a subset of nodes during training, randomly choosing the removed units at each training epoch. The reason is that it was observed that while training a network, after overfitting, the weights for some of neurons increases and cause the network to be dependant on them. By exploiting dropout, we are not dependant on any node anymore due to it is possible to drop it while training.
...
Finally, we decide to judge the goodness of our model in predicting the true values using a score called \emph{coefficient of determination} denoted as $R^{2}$ pronounced ``R squared''.

A data set has \emph{n} values marked $y_{1}$,...,$y_{n}$ each associated with a fitted (or modeled, or predicted) value $f_{1}$,...,$f_{n}$. Define residuals as $e_{i} = y_{i} - f_{i}$. If $\bar{y}$ is the mean of the observed data:
\begin{equation}
 \bar{y} = \frac{1}{n} \sum_{i=1}^{n}y_{i}
\end{equation}

then the variability of the data set can be measured with two \emph{sums of squares} formulas: the sum of squares of residuals $SS_{res}=\sum_{i}(y_{i}-f_{i})^{2} = \sum_{i} e_{i}^{2}$ and the total sum of squares (proportional to the variance of the data) $SS_{tot}=\sum_{i}(y_{i}-\bar{y})^{2}$.
...
\chapter{Regressor ANN at work}\label{ANNatwork}
We generate the \emph{k} parameters values that are distributed according to a Gamma distribution. This choice is due to the fact that we recognize these values as the most probable to be for the constant rates of reactions.

The probability density function for gamma is \cite{scipy}:
\begin{equation}
 f(x,a) = \frac{x^{a-1}e^{-x}}{\Gamma(a)}
\end{equation}
for $x \geq 0$, $a \ge 0$. Here, $\Gamma(a)$ refers to the Gamma function. \emph{a} is the shape parameter.
...
Finally, before starting, let us keep in mind that we generate 1000 autocorrelation data calculated with 5000 lags (relative to the SSA simulations time series results). Each set is refered to particular molecular species. Hence we consider three cases: the RNAS+Proteins autocorrelation case, the RNAs and proteins autocorrelations considered separately.
...
In particular we decide to use a batch size equal to 128. This is a trade off between a low batch size that would make the simulation much slower and high batch size that would require more memory storage and, finally, because it is the most suggested value to use in websites discussions. This last reason may seem a bit ``superficial'' but in this field experience from others help in the study. And we trained untill 50 epochs.  



\section{ANN applied to the first model}
Let us consider the first genetic model described in section \ref{regulatedgeneexpressionsection} with regulation that tends to keep the gene more active than inactive, i.e. $k_{a}=1$ and $k_{i}=0.5$.
...
What we can notice is that test data (never seen by the neural network) are sparse but most of these are close or lie on the diagonal (Fig.\ref{firstmodeltestdata0.05RNAPROTEINS}).

In this case $R_{test}^{2}=0.5$ and $R_{train}^{2}=0.9$. The score for the train is much higher than the one for the test. This maybe due to overfitting as suggested by the shape of the learning curve in Fig.\ref{firstmodellearningcurveRNAPROTEINS}.

Since the test set is 5\%, one attempt to get better performance was to increase the test set to 10\%. This produces infact learning curves with a smaller gap between the training loss curve and the test loss curve (see Fig.\ref{firstmodeltestdata0.10learningcurvesshowloweroverfittingRNAPROTEINS}). The the scores in this case are $R_{test}^{2}=0.4$ and $R_{train}^{2}=0.7$.

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.50]{firstmodeltestdata0.05RNAPROTEINS.png}
\caption{True data vs predicted data for the test set. RNAs and proteins autocorrelation are considered.}
\label{firstmodeltestdata0.05RNAPROTEINS}
\end{figure}

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.50]{firstmodeltraindata0.05RNAPROTEINS.png}
\caption{True data vs predicted data for the train set. RNAs and proteins autocorrelation are considered.}
\label{firstmodeltraindata0.05RNAPROTEINS}
\end{figure}

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.50]{firstmodellearningcurveRNAPROTEINS.png}
\caption{Learning curve of the ANN refered to RNA and proteins autocorrelations as input.}
\label{firstmodellearningcurveRNAPROTEINS}
\end{figure}

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.50]{firstmodeltestdata0.10learningcurvesshowloweroverfittingRNAPROTEINS.png}
\caption{Learning curve of the ANN refered to RNA and proteins autocorrelations as input. The test set is 10\%.}
\label{firstmodeltestdata0.10learningcurvesshowloweroverfittingRNAPROTEINS}
\end{figure}
\newpage
...
In the autoreprepressor model case, we start considering the values of the scores summerized in table \ref{tableautorepressorr2scores} without annoying the reader with all the predicted vs true paramaters plots and learning curves that, anyway, are reported in Appendix.
...
\begin{tabular}{ |c|c|c| } 
 ...
 \hline
\end{tabular}
\caption{\label{tableautorepressorr2scores} Autorepressor $R^{2}$ values for test data.}
\end{center}
\end{table}
...
Even in this case we report the table \ref{tabletoggleswitchr2scores} of the $R^{2}$ values. Since this system is made of two genes, each one producing their number of RNA and protein molecules, we decided to consider the autocorrelation relative to the RNAs molecules of both genes and the same is for proteins. 
...
\begin{tabular}{ |c|c|c| } 
 ...
 \hline
\end{tabular}
\caption{\label{tabletoggleswitchr2scores} Toggle switch $R^{2}$ values for test and  train data.}
\end{center}
\end{table}

In this case, RNAs + proteins autocorrelation values given as input is not reported because of lack of computational memory in order to do the simulation. However, as in the first model and autorepressor case, we should expect that they carry more information about the distribution of model parameters.
...
Anyway, leaving the biological discoveries apart, in all these genetic models, we can notice that the network is learning, specially from the relative loss curves (all reported in Appendix C) and $R^{2}$ values. This opens the way to an alternative method for the paramters distribution estimation.
...
\chapter{The NF-\texorpdfstring{$\kappa$}{\265}B family of transcription factors}\label{NFkBBiologyExplained}
...
\begin{itemize}
 \item $IkB\alpha_{activation} = k2 (NFkB)(IkB\alpha_{inactive})$
 
 \item $IkB\alpha_{deactivation} = k2i (IKKa)(A20_{active})$
...
\chapter{Results and discussion about the NF-kB model simulation}\label{themodelsimulation}
...
We can start to ``put our model under test'' by switching off one of the most studied inhibitor, namely I$\kappa$B$\alpha$. Hence we consider the model constant rates previously describe but with $k2=0$, $k2i=0$ when simulating the I$\kappa$B$\alpha$ knock-out. We report the plot of only the autocorrelation calculated with quantiles using 64 simulations in Fig.\ref{nfkbACFk20k2i0nsimulations64}.

We can notice that the medians points lie much more on the zero horizontal line and this is a sign of a lower oscillating signal and thus a more constituive expression behavior. In Appendix D the time trend of a SSA simulation made using this parameter is shown together with the case in which we put also $k3=0$, $k3i=0$ (see respectively Fig.\ref{nfkbtimeplotk2andk2i0} and Fig.\ref{nfkbtimeplotk20k2i0k30k3i0}). A brief comment about them: one can notice an oscillating behavior at the beginning and then there is a dampening. This is what we see by eye looking at the time series but the autocorrelation computed with quantiles for the case in which only k2 and k2i are equal to zero shows a general less oscillating behavior, as just said. 

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.70]{nfkbACFk20k2i0nsimulations64.png}
\caption{Autocorrelation curve plotted calculating quantiles of 64 simulations at each time step. Number of lags equal to 10000 and sampling period dt=10. The y-axis reports the autocorrelation values and on the x-axis is the sampling period. In this case $k2=0$, $k2i=0$.}
\label{nfkbACFk20k2i0nsimulations64}
\end{figure}


\newpage

Now coming back to the original model with all constant rates paramters different from zero, as in the case of the ``standard'' genetic models analysed, from the Fourier Transform analysis (Fig.\ref{nfkbFFTsamplingperiod0.01}) we can notice relevant peaks at low frequencies and the noise is evenly sparse towards higher frequencies  suggesting the presence of a periodicity also in this case.
...
However if we observe it, we can notice two dark red spots at about 600 and 1000 arbitrary units of time and other regular patterns for example some ``lighter openings'' between them. All located at low frequencies as exprected from the FFT analysis.
...
\chapter{Conclusions}
All this thesis work is triggered by the desire to give a mathematical modeling of the NF-$\kappa$B activity, that has a fundamental role in the regulation of many genes but also its disfunction leads to a long list of illnesses such as the colonrectal cancer. 
...
We ended this journey leaving a simple model of the NF-$\kappa$B activity based on the only actions of two inhibitors, the I$\kappa$B$\alpha$ and the A20. The results of this very simple model based only on first order reaction rate and ON/OFF behavior show a temporal series that has an oscillating behavior very similar to the one found by Hoffman et al. in their experiments. The autocorrelation function is indeed oscillating whereas the fast fourier tranform shows periodicity at low frequencies such as the first simple models analysed. From the continous wavelet transform some patterns arise but with the computational resources at hand it is difficult to draw conclusions. 

The model lacks of many features such as catalytical reactions and so second order reactions. However, just in the introduction of this thesis we mentioned the famous phrase ``all models are wrong but some are usefull''. Our model is wrong but it is usefull as a starting point and also to predict what happens if one of the few players involved lacks. For instance the important role played by the inhibitor I$\kappa$B$\alpha$ whose lacking leads to a constituive expression of NF-$\kappa$B, that is what happens in colon cancer cells.

And, also in this case as mentioned in the introduction, once the model agrees with the experimental data all that remains is finding the model parametrs, an hope for finding the cure for illness.

The neural networks analysis has shown to work, given the autocorrelation functions as input, in the three simple genetic models considered at the begging and not only this. It reveals also the amount of information stored in each chemical species. For instance in the autorepressor case, our neural network works better when the autocorrelations given as input are those of RNA molecules and less for proteins. 

%Now, we hope that these findings coming from simulations will be ``materialized'' through experimental data.
...
\begin{appendices}
  ...
  \chapter{Artificial Neural Networks plots}
    
    %FIRST MODEL PROTEINS
    
    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{firstmodeltestdata0.05PROTEINS.png}
    \caption{First model true data vs predicted data for the test set. Only proteins autocorrelation are considered as input.}
    \label{firstmodeltestdata0.05PROTEINS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{firstmodeltraindata0.05PROTEINS.png}
    \caption{First model true data vs predicted data for the train set. Only proteins autocorrelation are considered as input.}
    \label{firstmodeltraindata0.05PROTEINS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{firstmodellearningcurvePROTEINS.png}
    \caption{First model learning curve of the ANN refered to only proteins autocorrelations as input.}
    \label{firstmodellearningcurvePROTEINS}
    \end{figure}
    
    
    %AUTOREPRESSOR RNAS+PROTEINS
    
    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressortestdatsRNASPROTEINS.png}
    \caption{True data vs predicted data for the test set in case of autorepressor model. RNA+PROTEINS autocorrelations are considered as input.}
    \label{autorepressortestdatsRNASPROTEINS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressortraindataRNASPROTEINS.png}
    \caption{True data vs predicted data for the train set in case of autorepressor model. RNA+PROTEINS autocorrelations are considered as input.}
    \label{autorepressortraindataRNASPROTEINS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressorlearningcurveRNASPROTEINS.png}
    \caption{Learning curve of the ANN refered to only proteins autocorrelations as input for the autorepressor model.}
    \label{autorepressorlearningcurveRNASPROTEINS}
    \end{figure}
    ...
    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressortestdataRNASPROTEINSDROPOUT.png}
    \caption{True data vs predicted data for the test set in case of autorepressor model. RNA+PROTEINS autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{autorepressortestdataRNASPROTEINSDROPOUT}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressortraindatRNASPROTEINSDROPOUT.png}
    \caption{True data vs predicted data for the train set in case of autorepressor model. RNA+PROTEINS autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{autorepressortraindatRNASPROTEINSDROPOUT}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressorlearningcurveRNASPROTEINSDROPOUT.png}
    \caption{Learning curve of the ANN refered to only RNAS+PROTEINS autocorrelations as input for the autorepressor model. Dropout is added in the Neural Network model. }
    \label{autorepressorlearningcurveRNASPROTEINSDROPOUT}
    \end{figure}
    ...
    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressortestdataRNAS.png}
    \caption{True data vs predicted data for the test set in case of autorepressor model. RNA autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{autorepressortestdataRNAS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressortraindataRNAS.png}
    \caption{True data vs predicted data for the train set in case of autorepressor model. RNA autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{autorepressortraindataRNAS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressorlearningcurveRNAS.png}
    \caption{Learning curve of the ANN refered to only RNA autocorrelations as input for the autorepressor model. Dropout is added in the Neural Network model. }
    \label{autorepressorlearningcurveRNAS}
    \end{figure}    
    
    %AUTOREPRESSOR PROTEINS
    
    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressortestdatapredictedvstruePROTEINS.png}
    \caption{True data vs predicted data for the test set in case of autorepressor model. PROTEINS autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{autorepressortestdatapredictedvstruePROTEINS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressortraindatapredictedvstruePROTEINS.png}
    \caption{True data vs predicted data for the train set in case of autorepressor model. PROTEINS autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{autorepressortraindatapredictedvstruePROTEINS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{autorepressorlearningcurvePROTEINS.png}
    \caption{Learning curve of the ANN refered to only proteins autocorrelations as input for the autorepressor model. Dropout is added in the Neural Network model. }
    \label{autorepressorlearningcurvePROTEINS}
    \end{figure}    
    ...
    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{toggleswitchtestdatapredictedvstrueRNAS.png}
    \caption{True data vs predicted data for the test set in case of toggle switch model. RNA autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{toggleswitchtestdatapredictedvstrueRNAS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{toggleswitchtraindatapredictedvstrueRNAS.png}
    \caption{True data vs predicted data for the train set in case of toggle switch model. RNA autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{toggleswitchtraindatapredictedvstrueRNAS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{toggleswitchlearningcurveRNAS.png}
    \caption{Learning curve of the ANN refered to only RNA autocorrelations as input for the toggle switch model. Dropout is added in the Neural Network model. }
    \label{toggleswitchlearningcurveRNAS}
    \end{figure}
    
    
    
    %TOGGLE SWITCH PROTEINS
    
    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{toggleswitchtestdatapredictedvstruePROTEINS.png}
    \caption{True data vs predicted data for the test set in case of toggle switch model. PROTEINS autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{toggleswitchtestdatapredictedvstruePROTEINS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{toggleswitchtraindatapredictedvstruePROTEINS.png}
    \caption{True data vs predicted data for the train set in case of toggle switch model. PROTEINS autocorrelations are considered as input. Dropout is added in the Neural Network model.}
    \label{toggleswitchtraindatapredictedvstruePROTEINS}
    \end{figure}

    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{toggleswitchlearningcurvePROTEINS.png}
    \caption{Learning curve of the ANN refered to only PROTEINS autocorrelations as input for the toggle switch model. Dropout is added in the Neural Network model. }
    \label{toggleswitchlearningcurvePROTEINS}
    \end{figure}
    
  \chapter{NF-kB activity time series}
  
    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{nfkbtimeplotk2andk2i0.png}
    \caption{Number of RNA molecules produced through the NF-$\kappa$B activation and inhibition simulated by using the Stochastic Simulation Algorithm and considering the model in Fig.\ref{NFkBCompleteScheme} with parameters $ka=1$,$ki=0.1$,$k1=1$,$k1i=0.1$,$k4=1$,$k5=0.1$,$k3=0.1$,$k3i=1$ but considering $k2=0$ and $k2i=0$ starting from a configuration where everything is inactive.}
    \label{nfkbtimeplotk2andk2i0}
    \end{figure}  
    
    \begin{figure}[!ht]
    \centering
    \includegraphics[scale=0.50]{nfkbtimeplotk20k2i0k30k3i0.png}
    \caption{Number of RNA molecules produced through the NF-$\kappa$B activation and inhibition simulated by using the Stochastic Simulation Algorithm and considering the model in Fig.\ref{NFkBCompleteScheme} with parameters $ka=1$,$ki=0.1$,$k1=1$,$k1i=0.1$,$k4=1$,$k5=0.1$, but considering $k2=0$, $k2i=0$, $k3=0$, $k3i=0$ starting from a configuration where everything is inactive.}
    \label{nfkbtimeplotk20k2i0k30k3i0}
    \end{figure}  
    
    %L'oscillazione c'� comunque all'inizio fino a circa 600 abbiamo tipo un picco in tutti e 3 i casi poi nei casi degli inibitori messi fuori gioco abbiamo un dampening. L'oscillazione iniziale pu� essere dovuta al fatto che anche se i rate di attivazione non sono pi�  proporzionali ai fattori di tipo repressivo c'� comunque una proporzionalit� dei rate di attivazione e inattivazione degli IKK e NFkB con il loro stato di inattivazione e attivazione rispettivamente che fa in modo di attivare ed inattivare l'NF-kB per� in un modo meno marcato come evidenziato dalle curve di autocorrelazione.
...
Last, but not least, I acknowledge my family for their continuous love and support. I aknowledge also my friends for their encouragements.

After all, if we are what we are and we achieve what we achieve is also due to the environment in which we live and so to the people with which we interact (as explained in Chapter \ref{Heterogeneityinbiology}).
...
\begin{thebibliography}{bibliography}
    ...
    \bibitem{Example} Tomasz Lipniacki, Pawel Paszek, Allan R. Brasier, Bruce Luxon, Marek Kimmel, {\em Mathematical model of NF-kB regulatory module}, Journal of Theoretical Biology, Volume 228, Issue 2, 2004, Pages 195-215, ISSN 0022-5193.
    ...
    \bibitem{ANNBrainTumorsNMR} H. H. Sultan, N. M. Salem and W. Al-Atabany, {\em Multi-Classification of Brain Tumor Images Using Deep Neural Network}, in IEEE Access, vol. 7, pp. 69215-69225, 2019.
    \bibitem{MAE} (2011). Mean Absolute Error. In: Sammut, C., Webb, G.I. (eds) Encyclopedia of Machine Learning. Springer, Boston, MA.
    \bibitem{scipy} scipy.stats.gamma \href{https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.gamma.html}{https://docs.scipy.org}
