...
%I tre principali circuiti analizzati sono semplici da modellizzare però rappresentano importanti sistemi biologici e la loro simulazione e analisi non è affatto banale: serve anche qui molto succo di cervello e fantasia per poter svelare importanti caratteristiche che tali sistemi nascondono. I risultati ottenuti possono essere sfruttati in sistemi più articolati come l'NF-kB che è il nostro target. 
...
In this thesis work we will use the Stochastic Simulation Algorithm because our hybrid approach needs to be further tested and improved.



%L'algoritmo sembra funzionare meglio quando c'è una maggiore produzione di molecole (controlla proprio cosa succede: quante volte andiamo a finire in un if e quante in un altro).
...
%Metti il dogma centrale della biologia. Si tratta di un sistema stiff con le proteine che si producono più velocemente perché ad ogni RNA corrisponde una proteina o più proteine ? Mentre invece da un solo singolo gene partono più molecole di RNA. Infatti il tempo di warm up nel caso delle proteine è maggiore del caso dell'RNA che raggiunge uno stato stazionario che ha una media minore. 
%Sito.
...
%In questo caso dal punto di vista biologico non può succedere perchè i nostri geni non degradano.
...
%Sono applicazioni della Law of Mass Action.
%Inizia riassumendo la lezione del Professore.
...
%Dunque mentre nelle prime due configurazioni descritte abbiamo che troviamo questi sistemi tipicamente in determinate situazioni, nel caso dell'autorepressore e toggle-switch non ci sono proprio situazioni tipiche però nelle situazioni trovate da me sono tipicamente in situazioni in cui sono coinvolti gli oncogenes.

%Useremo come tempo limite 4000 per tutte le analisi perché come abbiamo visto nel Toggle Switch è il minimo tempo per poter osservare l'alternanza tra le espressioni e per avere una distribuzione Poissoniana nel caso di primo modello di sintesi proteica.
...
%Metti discorsi fatti dal 2 Novembre in poi e le novità sull'autocorrelazione delle differenze.
%La trasformata di Fourier media ed analisi di ogni picco per trovare il rate con cui sono prodotte le molecole e le wavelet transform. Usiamo tale modello perchÃ¨ Ã¨ il piÃ¹ semplice quindi come Toy model. 
%Autocorrelazione delle differenze.
%Questo lavoro di caratterizzare la forma di una funzione di autocorrelazione per ogni modello può essere utile nel momento in cui ho i dati sperimentali, ne calcolo l'autocorrelazione e se vedo un deep per esempio posso dire che c'è una forma di repressione.
...
%sfrutta i metodi adottati nei circuiti genetici più semplici.
...
%Usiamo i metodi per esplorare la biologia però vale anche il viceversa ossia la biologia stessa ci può aiutare a sviluppare i metodi come abbiamo fatto nell'algoritmo ibrido sfruttando le diverse configurazioni del modello ed i diversi tipi di regolazione.
\begin{appendices}
