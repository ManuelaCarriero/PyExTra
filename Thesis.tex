...
%I tre principali circuiti analizzati sono semplici da modellizzare perÚ rappresentano importanti sistemi biologici e la loro simulazione e analisi non Ë affatto banale: serve anche qui molto succo di cervello e fantasia per poter svelare importanti caratteristiche che tali sistemi nascondono. I risultati ottenuti possono essere sfruttati in sistemi pi˘ articolati come l'NF-kB che Ë il nostro target. 
...
\renewcommand{\chaptermark}[1]{%
  \markboth{\@chapapp \thechapter\ --\ #1}{\@chapapp \thechapter\ -- \ #1}% chapter/appendix name and number moved from \fancyhead[L]{Ö}
}
...
%Il fatto che cellule diverse possono avere stesso patrimonio genetico porta a pensare che non esiste solo il genetic heterogeneity bensÏ la causa di questa diversit‡ fenotipica puÚ essere data dall'environment e dal caso.
...
%Per√≤ ci sono processi come quelli descritti nell'articolo using gene expression noise for understanding gene regulation che dimostrano che i nostri geni non sono tutto.
%Single-cell experiments.
%Su cosa tratter√† l'intera tesi.
...
%Descrizione generale della master equation e dove √® usata in Fisica ispirandoti anche all'introduzione di Wikipedia + equazione di the Schl√∂gl reaction system.
...
%I numeri random che ci permettono di simulare quello che in natura abbiamo chiamato la ``stocasticit√†'' vanno generati in modo preciso.
...
\end{equation}
This is the differential form of the Chapman‚ÄìKolmogorov equation that is known as the \emph{Master equation}. It can be written in the simplified and more intuitive form
...
%METTICI PI˘ FORMULE PER DESCRIVERE OGNI STEP. Inoltre specifica che questi metodi servono sia per cercare un modo per velocizzare la simulazione (cosa che Ë molto importante) perÚ anche per attestare che i risultati che otteniamo sono giusti ossia che i metodi funzionano. Dunque bisogna controllare le autocorrelazioni dei diversi metodi con le incertezze e verificare che sono uguali.+ Nota che la configurazione dell'autorepressore Ë il peggio che possiamo chiedere al Tau-leap e l'efficienza di questi metodi (ossia il tempo impiegato per fare la simulazione) appunto dipende molto dalla configurazione che stiamo considerando.
...
%L'algoritmo sembra funzionare meglio quando c'Ë una maggiore produzione di molecole (controlla proprio cosa succede: quante volte andiamo a finire in un if e quante in un altro).
...
%Metti il dogma centrale della biologia. Si tratta di un sistema stiff con le proteine che si producono pi˘ velocemente perchÈ ad ogni RNA corrisponde una proteina o pi˘ proteine ? Mentre invece da un solo singolo gene partono pi˘ molecole di RNA. Infatti il tempo di warm up nel caso delle proteine Ë maggiore del caso dell'RNA che raggiunge uno stato stazionario che ha una media minore. 
%Sito.
...
%In questo caso dal punto di vista biologico non puÚ succedere perchË i nostri geni non degradano.
...
<=> \sigma S + rT}
 \end{equation}
where A and B are the reagents and S and T are the products of the reaction. This reaction is reversible and, as such, according to the Law of Mass Action, we can define a \emph{forward reaction rate} and a \emph{backward reaction rate}. The reaction rates are related to a constant, for instance the \emph{forward reaction rate} is related to $k_{+}$, and to the concentration of reagents:
\begin{equation}
...
%Dunque mentre nelle prime due configurazioni descritte abbiamo che troviamo questi sistemi tipicamente in determinate situazioni, nel caso dell'autorepressore e toggle-switch non ci sono proprio situazioni tipiche perÚ nelle situazioni trovate da me sono tipicamente in situazioni in cui sono coinvolti gli oncogenes.

%Useremo come tempo limite 4000 per tutte le analisi perchÈ come abbiamo visto nel Toggle Switch Ë il minimo tempo per poter osservare l'alternanza tra le espressioni e per avere una distribuzione Poissoniana nel caso di primo modello di sintesi proteica.
...
%I nostri occhi sono degli ottimi pattern recognizer infatti nell'autorepressore possiamo percepire che c'√® qualcosa che si ripete. Abbiamo per√≤ bisogno di altri mezzi perch√® il segnale nel tempo pu√≤ nascondere molte features interessanti under the hood. Queste non sempre possono essere svelate dall'autocorrelazione, che come detto nel paragrafo precedente ha come propriet√† di cogliere segnali che si ripetono periodicamente nel tempo. Per altri casi possiamo pensare di utilizzare: Transformata di Fourier + quelli elencati dal professore.
...
%La trasformata di Fourier media ed analisi di ogni picco per trovare il rate con cui sono prodotte le molecole e le wavelet transform. Usiamo tale modello perch√® √® il pi√π semplice quindi come Toy model. 
%Autocorrelazione delle differenze.
%Questo lavoro di caratterizzare la forma di una funzione di autocorrelazione per ogni modello puÚ essere utile nel momento in cui ho i dati sperimentali, ne calcolo l'autocorrelazione e se vedo un deep per esempio posso dire che c'Ë una forma di repressione.
...
%Noi usiamo l'autocorrelazione perÚ possono essere usate altre funzioni dipendenti dai parametri del sistema. Tipo alcuni usano lo scaleogramma. Trova le fonti che interessava molto il professore. 
...
%Confrontare la mae e la r2score generati da modelli uguali con diverse specie chimiche pu√≤ darci informazioni su tali specie.
...
%In unstimulated cells, NF-$\kappa$B proteins are sequestered in the cytoplasm through binding to inhibitory molecules (i.e. the I$\kappa$Bs family) whose prototype is I$\kappa$B$\alpha$. Stimulation with a variety of stimuli, such as proinflammatory cytokines (e.g., TNFa and IL-1b), Toll-like receptor ligands [e.g., lipopolysaccharide (LPS) and double-stranded RNA (dsRNA)], triggers NF-$\kappa$B activation through the so called ‚Äòclassical‚Äô or ‚Äòcanonical‚Äô pathway. This signaling pathway leads to the IKK complex, composed of both kinases IKK$\alpha$ and IKK$\beta$ assembled by the scaffold protein NEMO/IKK$\gamma$. 
...
conserved types, according to the two multiprotein IKK complexes which regulate the stimulus-responsive phosphorylation and consequent degradation of I$\kappa$B proteins: the classical or so-called ‚Äúcanonical‚Äù NF-$\kappa$B signaling pathway where IKK$\beta$ is sufficient for I$\kappa$B$\alpha$ and
...
alternative or ‚Äúnon-canonical‚Äù pathway IKK complex consists exclusively of an IKK$\alpha$ homodimer, which leads to the phosphorylation of p100 and its consequent processing to p52.
...
\end{figure}
\newpage
%The ‚Äòalternative‚Äô or ‚Äònon-classical‚Äô NF-$\kappa$B-activating pathway is triggered by cytokines such as BAFF and lymphotoxin-b, and leads to the activation of an IKK$\alpha$ homodimer, which phosphorylates p100. This inhibitory molecule is subsequently processed to generate p52. NF-$\kappa$B dimers (p52/RelB) move into the nucleus to drive the expression of candidates involved in adaptive immunity, as well as in lymphoid organogenesis.
In the canonical pathway, after pathogen-derived lipopolysaccharide (LPS) mediates inflammatory stimulus and cytokines such as TNF and interleukin (IL)-1, , the I$\kappa$B proteins I$\kappa$B$\alpha$, I$\kappa$B$\beta$ and I$\kappa$B$\epsilon$ are phosphorylated by the ‚Äúcanonical‚Äù IKK complex on two specific N-terminal serines.
...
Conversely, multiple non-inflammatory stimuli are responsible for the engagement of the ‚Äúnon-canonical‚Äù pathway in a variety of cell types.
...
%Per quanto riguarda l'algoritmo ibrido, occorrerebbe testarlo su tantissimi altri modelli e se ne ricaverebbe sicuramente miglioramento e automatizzare la scelta della differenza dei rate di soglia.
...
%Usiamo i metodi per esplorare la biologia perÚ vale anche il viceversa ossia la biologia stessa ci puÚ aiutare a sviluppare i metodi come abbiamo fatto nell'algoritmo ibrido sfruttando le diverse configurazioni del modello ed i diversi tipi di regolazione.
...
\begin{thebibliography}{bibliography}
    ...
    \bibitem{Tesi} Davide Giosu√© Lippolis, {\em Stochastic modeling of fluctuations in the NF-$\kappa$B activity of neoplastic cells}, University of Bologna (2018/2019).
    \bibitem{VanKampenBook} N. G. van Kampen, {\em Stochastic Processes in Physics and Chemistry}, North Holland, third ed., May 2007.
    \bibitem{Genetics} Yifeng Xia, Shen Shen, and Inder M. Verma, {\em NF-$\kappa$B, an active player in human cancers}, Cancer Immunol Res (2014) 2 (9): 823‚Äì830.
    ...
    \bibitem{Article} Tam√°s Sz√©kely Jr., Kevin Burrage, {\em Stochastic simulation in systems biology}, Computational and Structural Biotechnology Journal, Volume 12, Issues 20‚Äì21, November 2014, Pages 14-25.
    ...
    \bibitem{onesteparticle} Mathisca de Gunst, Michel Mandjes and Birgit Sollie, {\em Statistical inference for a quasi birth‚Äìdeath model of RNA transcription}, BMC Bioinformatics 23, 105 (2022).
    ...
    %\bibitem{Exp} Baeuerle, P. A. Baltimore, D. A 65-kappaD subunit of active NF-kappaB is required for inhibition of NF-kappaB by I kappaB. Genes Dev. 3, 1689‚Äì1698 (1989).
