...
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.70]{SSA55.25sTauleap10.22stimelimit9000.png}
...
Let us consider our usual state of molecular populations $\Vec{Y}$ and the initial state $\Vec{Y}=\Vec{Y_{0}}$. The transition rates related to such state is $\Vec{\mathbf{K}}=\Vec{\mathbf{K_{0}}}$.
...
This process is iterated untill the $\tau^{(n)}$ reaches a value that is higher than 3 times the characteristic time given by the SSA by considering all reactions happening in our system.

\item If the new $\tau^{(n)}$ reaches a lower value, it is worth to make a SSA step without getting trapped in the previous iterating system that is certainly less precise than the SSA. In this case, update the total time with the characteristic time given by the SSA applied considering all the reactions that are possible in our system.
