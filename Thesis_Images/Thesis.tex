...
Human brain is full of neurons and so an artificial neural networks if made of many computational units that play the role of neurons. A biological \emph{neuron} receives inputs, transforms it and relays to the other at every step. In biophysical terms, this signal is the action potential that is an electrical signal transmitted by a neural cell across the \emph{axon}. 
...
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.70]{Neuron.png}
...
The nodes in the input layer do not perform any numerical processing. All numerical processing is done by the hidden and output layer nodes. Fig.\ref{MFFN} shows the structure of a typical multiinput-multioutput neural network.
...
\chapter{Conclusions}
All this thesis work is triggered by the desire to give a mathematical modeling of the NF-$\kappa$B activity, that has a fundamental role the regulation of many genes but also its disfunction leads to a long list of illnesses such as the colonrectal cancer cell. 
