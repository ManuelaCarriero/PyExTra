...
We are going to use this to evaluate the true vs predicted data since the better the linear regression fits such data, the closer is the value of $R^{2}$ is to 1.
...
%Teniamo i parametri costanti per fare un confronto tra diversi modelli e generiamo i dati con una distribuzione Gamma.
...
%\caption{True data vs predicted data for the train set. Only proteins autocorrelation are considered as input.}
...
%\caption{Learning curve of the ANN refered to only proteins autocorrelations as input.}
%\label{firstmodellearningcurvePROTEINS}
%\end{figure}

\section{ANN applied to the autorepressor model}

In the autoreprepressor model case, we start considering the values of the scores summerize in table \ref{tableautorepressorr2scores}.
