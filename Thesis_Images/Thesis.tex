...
In all the three domains of life, there are two main stages from DNA sequence of a gene to protein: transcription, that is the process of making a strand of RNA that is complementary to a single strand DNA, and translation, that is the process by which a protein is synthesized from the information contained in a molecule of messenger RNA (mRNA). In the process of transcription, proteins called ``transcription factors'' play a central role because they bind to a DNA sequence and help the transcription initiation complex. In eukaryotic cells, other transcription factors bind to regulatory sequences, such as enhancers and silencers that can respectively stimulate and repress transcription of the related gene. In this work of thesis, we are particularly interested in the simulation and analysis of the nuclear factor-kappa B (NF-$\kappa$B) activity which is ubiquitous within cells and its dysfunction leads to chronic diseases, cancers, neurodegenerative diseases and an other very long list of illnesses. In this thesis, however, we do not started immediately modeling its behavior, whose mathematical models made by researchers are already a high number. In this stochastic context, firstly we aim to review and deepen into stochastic simulation algorithms to solve the Chemical Master Equation (CME) giving an our alternative algorithm called ``hybrid'' because it combines the Gillespie's Stochastic Simulation Algorithm (SSA) with the tauleaping algorithm with the attempt to improve the algorithm's speed and, at the same time, preserve the SSA accuracy; secondly we analyse the stochastic simulation results of three basics genetic circuits (the simplest model of gene expression, the autorepressor and the toggle switch) with the help of the Autocorrelation Function, the Fast Fourier Transform and the Continous Wavelet Transform with the attempt to find interesting patterns from the temporal series; third, we faced the problem of parameters estimation of these simple genetic circuits using artificial neural networks rather than the often used Approximate Bayesian Computation (ABC) that has the limit to be computationally expensive; finally, 
conscientious of what we have learned after all such steps, we provide a very little NF-$\kappa$B model. The results for each step are the following: the hybrid algorithm applied to the first protein synthesis model configuration is faster than the SSA but in configurations where the number of molecules produced tends to be high; periodicity arises from the temporal series, in particular from the autorepressor case; the neural networks learn to predict the model parameters given the autocorrelation as input but the choice of the chemical species for autocorrelation calculation makes differences in results; finally, our little NF-$\kappa$B model that considers the contribution of only I$\kappa$B$\alpha$ and A20 as inhibitors in the simplified system, shows an oscillating behavior that is similar to the one predicted by Hoffman et al. in their experiments.   
\end{flushleft}
\thispagestyle{empty}
\clearpage
...
Following the line of reasoning of the previous section, cancers like colonrectal cancers are mainly caused by mutations that target oncogenes leading to novel or increased functions, or alterations that drive to loss of functions of tumor-suppressor genes. This provide neoplastic cells with a survival advantage over the surrounding 
...
It is very likely that these mutations lead to a malfunction NF-$\kappa$B transcription factor activity, whose behavior is tamed by its regulators, of the proteins family I$\kappa$B, but that in CRC it is constitutively expressed. 

The biology of NF-$\kappa$B will be explained in Chapter \ref{NFkBBiologyExplained} and then a simple model of the feedback loops which involve him will be furnished. However before arriving to this point, we will examine simple genetic models that will serves as a ``training'' before diving into more complex models, which are indeed stochastic.  

This will let us to explore \emph{methods} firstly for simulations, secondly for analyzing data coming from them and last, but not least, to extrapolate model paramaters if the simulations results agree with the experimental data.   
...
\chapter{Modeling kinetics reactions}\label{models}
...
\chapter{SSA simulation of genetic circuits}

In chapter \ref{models} we described in a deterministic frameworks the three simple genetic circuits at the basis of living organisms. Now that we have deepened into stachastics simulations, we can look at their time course behavior using the SSA algorithm.
...
This is not the case of proteins distribution. 
...
Anyway, leaving the biological discoveries apart, in all these genetic models, we can notice that the network is learning from the relative loss curves and $R^{2}$ values and this opens the way to an alternative method for the paramters distribution estimation.
...
We can notice that the medians points lie much more on the zero horizontal line and this is a sign of a lower oscillating signal and thus a more constituive expression behavior.
...
\addcontentsline{toc}{chapter}{Acknowledgments} 
I gratefully acknowledge the help and support of Professor Enrico Giampieri. His knowledge, experience and advices gave me the possibility to study on this thesis.
...
Last, but not least, I acknowledge my family for their continous love and support. I aknowledge also my friends for their encouragements.

After all, if we are what we are and we achieve what we achieve is also due to the environment in which we leave and so to the people with which interact (as explained in Chapter \ref{Heterogeneityinbiology}).
